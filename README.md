...
```bash
npm install @decpro/livesurf-sdk
```
...
```js
import { LiveSurfApi } from "@decpro/livesurf-sdk";

const api = new LiveSurfApi("ВАШ_API_КЛЮЧ");
...
- Таймаут и экспоненциальный backoff  

---

## ⚙️ Пример дополнительных настроек

```js
const api = new LiveSurfApi("API_KEY", {
  timeout: 20000,
  rateLimit: 5,
  maxRetries: 5,
